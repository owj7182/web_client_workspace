 ...
 * - 문자열에 대해서 유효성 검사/ 검색/ 대체등을 처리하기 위한 표현식
...
document.querySelector('#btn5').addEventListener('click', (e) => {
    ...
    area.innerHTML += `<p>${src.replace(/[abcdefghijklmnopqrsyuvwxyz]/g, '<mark>$&</mark>')}</p>`;
    ...
    // 유의사항 A ~ Z 중 Q를 제외하고 출력하고 싶을때
    // 반전을 의미하려면 ^은 맨첫째자리만 사용가능
    // /[A-Z^Q]/ 이런식으로 쓰면 안됨 원하는 값이 출력 안된다. 오류는 아님
    // /[A-PR-Z]/ 이런식으로 작성해야함
...
/**
 * 탐색 Look Around
 * - 탐색구문은 조회에 사용되고, 매칭 결과에는 반영되지 않는다.
 * 
 * - 전방탐색 Look ahead
 * - a(?=b) b가 뒤따르는 a를 검색
