...
$(btn10).click(() => {
    ...
    _data.forEach(({name, price}) => {
        ...
        $("<tr></tr>").append($("<td></td>").text(name)).append($("<td></td>").text(price)).prependTo($tbody); // 위에 코드랑 동치
    });
...
$(btn12).click(() => {
    ...
    console.log(
        $src
            // .children() // 직계 자식
            // .children(":has(ul)") // ul태그를 가진 자식태그
            .find("span") // 건너건너도 모두 찾아냄
            .css(style)
    );
});
 ...
 * 
 *  - next() 다음 형제요소 하나
 *  - nextAll([selectot]) 다음 형재요소 모두
 *  - nextUntil(selector) 특정태그 이전 형제요소
 * 
 *  - prev() 이전 형제요소 하나
 *  - prevAll([selector]) 이전 형제요소 모두
 *  - prevUntil(selector) 특정캐그 다음 형제요소
 */
$(btn13).click(() => {
